...
YGFree gYGFree = &free;
#else
YGMalloc gYGMalloc = NULL;
YGCalloc gYGCalloc = NULL;
YGRealloc gYGRealloc = NULL;
YGFree gYGFree = NULL;
#endif
...
// mycustom
WIN_EXPORT void YGNodeResetParent(const YGNodeRef node)
{
    if (node->parent != NULL) {
        node->parent = NULL;
    }
...
bool YGLayoutNodeInternal(const YGNodeRef node,
  ...
  if (!needToVisitNode && cachedResults != NULL) {
    ...
    if (gPrintChanges && gPrintSkips) {
      //printf("%s%d.{[skipped] ", YGSpacer(gDepth), (int)gDepth);
      if (node->print) {
        node->print(node);
      }
    ...
    if (gPrintChanges) {
      //printf("%s%d.{%s", YGSpacer(gDepth), (int)gDepth, needToVisitNode ? "*" : "");
      if (node->print) {
        node->print(node);
      }
